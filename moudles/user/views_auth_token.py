...
from moudles.user.serializers import UserSerializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if request.path_info.find("/login/") > 0:
                ...
                User.objects.filter(id=user.pk).update(last_login=now)
        ...
        if project_id:
            ...
            for ur in user_role:
                if user_permission_in_project:
                    user_permission_in_project = user_permission_in_project | Group.objects.filter(id=ur.group_id)[
                        0].permissions.all()
                else:
